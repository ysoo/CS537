...
main(int argc, char *argv[]) {
  ...
  int temp = fstat(inf, &filestat);
  if (temp < 0) {
    fprintf(stderr, "Error: Cannot get filesize\n");
    exit(1);
  }
...
printf("filesize : %d \n", filesize);
  ...
  for (int k = 0 ; k < recordsleft; k++) {
    ...
    rc = read(inf, &r, sizeof(rec_dataptr_t));
    if (rc != sizeof(rec_dataptr_t)) {
      perror("read");
      exit(1);
    }

    // Read the variable portion of the record
//    rc = read(inf, &r.data_ptr, r.data_ints*sizeof(unsigned int));
